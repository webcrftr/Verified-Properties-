      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl sm:text-5xl font-light font-serif text-white">
            What Our <span className="font-serif italic font-bold text-[#F4D35E]">Clients Say</span>
          </h2>
        ...
        <div className="hidden lg:grid grid-cols-3 gap-8">
          {TESTIMONIALS.map((test, index) => (
            <div
              key={index}
              className="bg-[#0a1122] p-8 border border-white/10 relative flex flex-col justify-between hover:border-[#FFD54F]/35 transition-all duration-300"
            >
              <div>
                {/* Gold Quote icon decoration */}
                <div className="absolute top-6 right-6 opacity-5">
                  <Quote className="h-10 w-10 text-[#F4D35E] rotate-180" />
                </div>

                {/* Rating layout */}
                <div className="flex items-center gap-1 mb-5">
                  {[...Array(test.stars)].map((_, i) => (
                    <Star key={i} className="h-4 w-4 text-[#F4D35E] fill-[#F4D35E]" />
                  ))}
                </div>
              ...
              <div className="border-t border-white/10 pt-5 mt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-[#F4D35E] font-mono uppercase tracking-wider mt-0.5">{test.role}</p>
                </div>
                <span className="px-2.5 py-1 bg-[#F7E7A1]/10 text-[#F7E7A1] text-[9px] font-bold uppercase tracking-wider flex items-center gap-1 border border-[#F7E7A1]/20">
        ...
        <div className="block lg:hidden max-w-xl mx-auto">
          <div className="bg-[#0a1122] p-6 sm:p-8 border border-white/10 relative min-h-[250px] flex flex-col justify-between">
            <div>
              ...
              <div className="absolute top-4 right-4 opacity-5">
                <Quote className="h-8 w-8 text-[#F4D35E]" />
              </div>
            ...
            <div className="border-t border-white/10 pt-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-[9px] text-[#F4D35E] font-mono uppercase tracking-wider mt-0.5">{TESTIMONIALS[currentIndex].role}</p>
              </div>
              <span className="px-2 py-0.5 bg-[#F7E7A1]/10 text-[#F7E7A1] text-[8px] font-bold uppercase tracking-wider flex items-center gap-1 border border-[#F7E7A1]/20">
                <BadgeCheck className="h-3 w-3" />
          ...
          <div className="flex justify-center gap-4 mt-6">
            <button
              onClick={handlePrev}
              className="p-3 bg-white/5 border border-white/10 hover:border-[#FFD54F] text-white active:scale-95 transition-all cursor-pointer"
