...
export default function ServicesSection() {
  const services = [
    {
      icon: <Home className="h-6 w-6 text-[#F0D36B]" />,
    ...
    {
      icon: <TrendingUp className="h-6 w-6 text-[#F0D36B]" />,
    ...
    {
      icon: <KeyRound className="h-6 w-6 text-[#F0D36B]" />,
    ...
    {
      icon: <ClipboardCheck className="h-6 w-6 text-[#F0D36B]" />,
    ...
    {
      icon: <Map className="h-6 w-6 text-[#F0D36B]" />,
    ...
    {
      icon: <Building2 className="h-6 w-6 text-[#F0D36B]" />,
  ...
  return (
    <section id="services" className="py-24 bg-[#111827] text-white border-b border-[#374151] relative overflow-hidden">
      
      {/* Absolute subtle ambient light */}
      <div className="absolute right-1/4 top-1/4 w-80 h-80 bg-[#F0D36B]/5 rounded-full filter blur-[150px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl sm:text-5xl font-light font-serif text-white">
            Our Consultant <span className="font-serif italic font-bold text-[#F0D36B]">Offerings</span>
          </h2>
          <div className="h-[1px] w-24 bg-[#E8C75A] mx-auto mt-6"></div>
          <p className="max-w-xl mx-auto text-xs sm:text-sm text-[#D1D5DB] mt-5 uppercase tracking-wider leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((svc, i) => (
            <div
              key={i}
              className="p-8 bg-[#1F2937]/90 backdrop-blur-md border border-[#374151] hover:border-[#D9B84C]/35 transition-all duration-300 flex flex-col justify-between group"
            >
              <div>
                {/* Icon Container with thin brass framing */}
                <div className="bg-[#111827] border border-[#374151] p-3.5 inline-flex mb-6 group-hover:border-[#D9B84C]/45 transition-all">
                  {svc.icon}
                </div>
                
                {/* Title */}
                <h3 className="text-xl font-light font-serif text-white mb-4 group-hover:text-[#F0D36B] transition-colors">
                  {svc.title}
                </h3>
                
                {/* Description */}
                <p className="text-[#D1D5DB] text-xs sm:text-sm leading-relaxed mb-8 font-sans">
                  {svc.details}
                </p>
              </div>

              {/* Minimal visual feedback link */}
              <div className="flex items-center gap-2 text-[10px] font-mono text-[#F0D36B] tracking-[0.15em] uppercase font-bold">
                <span>TAILORED CONSULTATION</span>
                <span className="w-1.5 h-1.5 bg-[#E8C75A]"></span>
              </div>
            </div>
          ))}
        </div>
